...
\begin{document}
	...
	\begin{itemize}
		...
		\item Człon 2 rzędu: $G(s) = \frac{1}{1 + sa_1 + s^2a_2} = \frac{1}{1+s 2\zeta \tau + s^2 \tau^2} = \frac{w_n^2}{w_n^2 + s 2 \zeta w_n + s^2}$, gdzie wyznaczone parametry to $\zeta = 0,37$ i $\omega_n = 2330 \text{ rad/s}$.
	\end{itemize}
	...
	\begin{figure}[H]
		...
		\caption{Odpowiedź skokowa układu (A) przy różnych wartościach wzmocnienia \(k_c\).}
		\label{fig:OdpSkokA}
	\end{figure}
	
	% TUTAJ WPISZ ANALIZĘ - np:
	% Z wykresu widać, że... Wzrost wzmocnienia kc powoduje...
	...
	\begin{figure}[H]
		\centering
		% \includegraphics[width=0.8\linewidth]{zdjecia/rlocus_A.png}
		\caption{Linie pierwiastkowe układu otwartego (A).}
		\label{fig:rlocus_A}
	\end{figure}
	
	% TUTAJ WPISZ ANALIZĘ - np:
	% Linie pierwiastkowe pokazują, że... Dla wzmocnienia k=... układ staje się niestabilny. 
